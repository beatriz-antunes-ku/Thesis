...
path <- "/Users/rnq676/Documents/thesis/KRAS - DARPin K27"

#Read in dataframes
data_crystal <- read.table(file.path(path, "prism_rosetta_XXX_5o2s_mut.txt"), header = TRUE)

data_mave <- read_excel(file.path("/Users/rnq676/Documents/thesis/KRAS/mavedb_4obe.xlsx"), 
                        sheet = 2) %>%
  select(aa_seq, fitness, sigma, WT, assay) %>%
  filter(assay == "BindingPCA DARPin K27")
...
ggplot(data, aes(x=fitness)) +
  ...
  labs(title = "Mave scores distribution for KRAS - DARPin K27", x = "MAVE Score", y = "Frequency") +
...
ggroc(list(roc_crystal = roc(data$cutoff_mave, data$mean_ddG)), 
  ...
  labs(title="ROC curve for KRAS - DARPin K27",x = "False Positive Rate", y = "True Positive Rate") +
...
data <- data %>%
  ...
  mutate(
    ...
    SASA = ifelse(Pos %in% c(21, 24, 25, 27, 29, 30, 31, 32, 33, 34, 35, 36, 38, 39, 40, 41, 43, 52, 54, 67, 70, 71), "Interaction", SASA),
...
ggplot(df_long, aes(x = fitness, y = mean_ddG, fill = SASA)) +
  ...
  labs(x = "MAVE Score", y = "ddG", title = "MAVE Scores vs Crystal ddG for KRAS - DARPin K27") +
...
title <- ggplot() +
  ggtitle("MAVE Scores from Abundance vs Interaction for KRAS - DARPin K27") +
  theme_void() +
...
title <- ggdraw() +
  draw_label(
    "MAVE Scores from Abundance vs Interaction for KRAS - DARPin K27", 
    hjust = 0.5
  )
