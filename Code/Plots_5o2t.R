...
#Path to data
path <- "/Users/rnq676/Documents/thesis/KRAS - DARPin K55"

#Read in data
data_crystal <- read.table(file.path(path, "prism_rosetta_XXX_5o2t_mut.txt"), header = TRUE)

data_mave <- read_excel(file.path("/Users/rnq676/Documents/thesis/KRAS/mavedb_4obe.xlsx"), 
                        sheet = 2) %>%
  select(aa_seq, fitness, sigma, WT, assay) %>%
  filter(assay == "BindingPCA DARPin K55")
...
ggplot(data, aes(x=fitness)) +
  ...
  labs(title = "Mave scores distribution for KRAS - DARPin K55", x = "MAVE Score", y = "Frequency") +
...
ggroc(list(roc_crystal = roc(data$cutoff_mave, data$mean_ddG)), 
  ...
  labs(title="ROC curve for KRAS - DARPin K55",x = "False Positive Rate", y = "True Positive Rate") +
...
data <- data %>%
  ...
  mutate(
    ...
    SASA = ifelse(Pos %in% c(5, 24, 25, 31, 33, 36, 37, 38, 39, 40, 54, 56, 64, 66, 67, 70, 73, 74), "Interaction", SASA),
...
ggplot(df_long, aes(x = fitness, y = mean_ddG, fill = SASA)) +
  ...
  labs(x = "MAVE Score", y = "ddG", title = "MAVE Scores vs Crystal ddG for KRAS - DARPin K55") +
...
title <- ggplot() +
  ggtitle("MAVE Scores from Abundance vs Interaction for KRAS - DARPin K55") +
  theme_void() +
...
title <- ggdraw() +
  draw_label(
    "MAVE Scores from Abundance vs Interaction for KRAS - DARPin K55", 
    hjust = 0.5
  )
