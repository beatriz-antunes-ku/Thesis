...
#Path to data
path <- "/Users/rnq676/Documents/thesis/KRAS - RAF1"

#Read in data
data_crystal <- read.table(file.path(path, "prism_rosetta_XXX_6vjj.txt"), header = TRUE)
data_alphafold <- read.table(file.path(path, "prism_rosetta_XXX_fold_6vjj.txt"), header = TRUE)
    ...
    mutate(variant = {
      ...
      changes <- map_chr(seq_along(wildSplit), function(i) {
        if (i <= length(mutSplit) && wildSplit[i] != mutSplit[i]) {
          paste0(wildSplit[i], i+2, mutSplit[i])
        } else {
...
ggplot(data, aes(x=fitness)) +
  ...
  labs(title = "Mave scores distribution for KRAS - RAF1", x = "MAVE Score", y = "Frequency") +
...
ggroc(list(roc_crystal = roc(data$cutoff_mave, data$mean_ddG.crystal), roc_alphafold = roc(data$cutoff_mave, data$mean_ddG.alphafold)), 
  ...
  labs(title="ROC curve for KRAS - RAF1",x = "False Positive Rate", y = "True Positive Rate") +
...
title <- ggplot() +
  ggtitle("Crystal Structure ddG vs Alphafold Prediction ddG for KRAS - RAF1") +
  theme_void() +
...
data <- data %>%
  ...
  mutate(
    ...
    SASA = ifelse(Pos %in% c(21, 25, 29, 31, 33, 36, 37, 38, 39, 40, 41, 67, 71), "Interaction", SASA),
...
title <- ggdraw() +
  draw_label(
    "Crystal Structure ddG vs Alphafold Prediction ddG for KRAS - RAF1", 
    hjust = 0.5
  )
...
ggplot(df_long, aes(x = fitness, y = ddg)) +
  geom_point(size = 1.5, shape = 21, fill = "dodgerblue2", color = "grey20", alpha = 0.5) +
  labs(x = "MAVE Score", y = "ddG", title = "MAVE Scores vs ddG for KRAS - RAF1") +
...
ggplot(df_long, aes(x = fitness, y = ddg, fill = SASA)) +
  ...
  labs(x = "MAVE Score", y = "ddG", title = "MAVE Scores vs Crystal ddG for KRAS - RAF1") +
...
title <- ggplot() +
  ggtitle("MAVE Scores from Abundance vs Interaction for KRAS - RAF1") +
  theme_void() +
  theme(plot.title = element_text(hjust = 0.5, size = 14))
...
title <- ggdraw() +
  draw_label(
    "MAVE Scores from Abundance vs Interaction for KRAS - RAF1", 
    hjust = 0.5
  )
